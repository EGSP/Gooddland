...
namespace Game.Cameras
{
    ...
    public class SceneCamera : SerializedMonoBehaviour, IDisposable
    {
        ...
        public void TranslateToTarget(SceneCamera target, CallBack callBack)
        {    
            if(translateRoutine != null)
                StopCoroutine(translateRoutine);

            translateRoutine = TranslateCameraToTarget(target.transform.position, smoothness, callBack);
            StartCoroutine(translateRoutine);
        }

        private IEnumerator TranslateCameraToTarget(Vector3 targetPosition, float smoothness, CallBack callBack)
        {
        ...
        public void TranslateToStart(bool immediately = false)
        {
            ...
            translateRoutine = TranslateCameraToTarget(startPosition, localSmoothness, new CallBack());
            StartCoroutine(translateRoutine);
        }
    }
}