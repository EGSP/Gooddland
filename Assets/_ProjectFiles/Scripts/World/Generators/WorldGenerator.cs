...
namespace World.Generators
{
    ...
    public class WorldGenerator : Singleton<WorldGenerator>
    {
        ...
        private IsoVisualGenerator isoVisualGenerator; // obsolete
        ...
        /// Карта аномалий.
        /// </summary>
        public Grid<float> AnomalyMap { get; private set; }
        
        /// <summary>
        /// Карта температуры.
        /// </summary>
        public Grid<float> HeatMap { get; private set; }
        
        /// <summary>
        /// Все существующие биомы.
        /// </summary>
        private List<Biome> Biomes { get; set; }

        /// <summary>
        /// Генерация данных о мире.
        /// </summary>
        public static void GenerateWorldInfo()
        {
            // Создание новых данных о мире.
            var worldProxy = Storage.l.GetProxy("/World_Generator/mapInfo");

            worldProxy.SetInt("anomalyMapSeed", UnityEngine.Random.Range(0, 1_000_000))
                .SetInt("heatMapSeed", UnityEngine.Random.Range(0, 1_000_000))
                .Apply();
        }
        ...
        public void LoadWorldInfo()
        {
            ...
            worldProxy.GetInt("anomalyMapSeed", ref anomalyMapSeed)
                .GetInt("heatMapSeed", ref heatMapSeed);

            AnomalyMap = mapGenerator.GenerateAnomalyMap(worldSize, worldSize);
            HeatMap = mapGenerator.GenerateHeatMap(worldSize, worldSize);
        }
        
        /// <summary>
        /// Генерация информации об острове на основе его координат в мире.
        /// </summary>
        public Island CreateIsland(int x, int y)
        {
            var island = new Island(x, y);
            island.AnomalyModifier = AnomalyMap[x, y];
            island.HeatModifier = HeatMap[x, y];
            island.SetSizes(islandLength, islandWidth);
        ...
        private void TestGenerateExtraMap()
        {
            ...
            var extraMap = mapGenerator.ApplySphereMask(
                mapGenerator.GenerateLandscapeMap(islandLength, islandWidth));
        ...
        private void TestGenerateIslandInfo(int x, int y)
        {
            ...
            var extraMap = mapGenerator.ApplySphereMask(
                mapGenerator.GenerateLandscapeMap(islandLength, islandWidth, seed+anomalyMapSeed));
